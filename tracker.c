...
static void tracker_draw_erase(t_tracker* x, t_glist* glist) {
    t_canvas* canvas = glist_getcanvas(x->x_glist);
    sys_vgui(".x%lx.c delete %xSHAPE\n", canvas, x);
    sys_vgui(".x%lx.c delete %xTEXT\n", canvas, x);
    sys_vgui(".x%lx.c delete %xCURSOR\n", canvas, x);
}

static void tracker_draw_select(t_tracker* x, t_glist* glist) {
    t_canvas* canvas = glist_getcanvas(x->x_glist);
    if(x->x_selected) {
        sys_vgui(".x%lx.c itemconfigure %xSHAPE -fill blue\n", canvas, x);
        sys_vgui(".x%lx.c itemconfigure %xTEXT -fill white\n", canvas, x);
        sys_vgui(".x%lx.c itemconfigure %xCURSOR -fill blue\n", canvas, x);
    } else {
        sys_vgui(".x%lx.c itemconfigure %xSHAPE -fill gray\n", canvas, x);
        sys_vgui(".x%lx.c itemconfigure %xTEXT -fill black\n", canvas, x);
        sys_vgui(".x%lx.c itemconfigure %xCURSOR -fill #dddddd\n", canvas, x);
    }
    if(x->x_active_row >= 0 && x->x_active_column >= 0)
        sys_vgui(".x%lx.c itemconfigure %xTEXT.%d.%d -fill red\n", canvas, x, x->x_active_row, x->x_active_column);
}
...
static void tracker_select_item(t_tracker* x, int r, int c) {
    ...
    if(r >= 0 && c >= 0) {
        if(r != x->x_active_row || c != x->x_active_column) {
            if(x->x_active_row >= 0 && x->x_active_column >= 0) {
                sys_vgui(".x%lx.c itemconfigure %xTEXT.%d.%d -fill black\n", canvas, x, x->x_active_row, x->x_active_column);
                tracker_reset_buffer(x);
                tracker_draw_update_single(x, x->x_active_row, x->x_active_column);
            }
            x->x_active_row = r;
